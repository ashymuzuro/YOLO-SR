...
def wasserstein_loss(pred_box, target_box, constant=12.0):
    """
    Calculates Normalized Wasserstein Distance (NWD) for tiny objects.
    pred_box, target_box: [x_min, y_min, x_max, y_max]
    """
    # Convert xyxy to xywh
    p_w = pred_box[:, 2] - pred_box[:, 0]
    p_h = pred_box[:, 3] - pred_box[:, 1]
    p_xc = pred_box[:, 0] + p_w / 2
    p_yc = pred_box[:, 1] + p_h / 2

    t_w = target_box[:, 2] - target_box[:, 0]
    t_h = target_box[:, 3] - target_box[:, 1]
    t_xc = target_box[:, 0] + t_w / 2
    t_yc = target_box[:, 1] + t_h / 2

    # Square the width and height to get covariance (approximation for Gaussian)
    p_w2, p_h2 = p_w**2, p_h**2
    t_w2, t_h2 = t_w**2, t_h**2
    
    # Calculate Center Distance Squared
    center_dist2 = (p_xc - t_xc)**2 + (p_yc - t_yc)**2
                   
    # Wasserstein Distance formula for diagonal Gaussian
    w2 = center_dist2 + ((p_w2 - t_w2)**2 + (p_h2 - t_h2)**2) / 4.0
    
    # Normalize (NWD)
    nwd = torch.exp(-torch.sqrt(w2.clamp(min=1e-6)) / constant)
    
    # Loss is 1 - NWD
    return 1.0 - nwd

# --- PhD Innovation 1: NWD Loss (For Tiny Objects) ---
def nwd_loss(pred_bboxes, target_bboxes, constant=12.0):
    """
    Normalized Wasserstein Distance (NWD) for tiny objects.
    Fixes the issue where non-overlapping tiny boxes have 0 gradient.
    """
    # bboxes: [x_center, y_center, w, h]
    p_x, p_y, p_w, p_h = pred_bboxes.unbind(-1)
    t_x, t_y, t_w, t_h = target_bboxes.unbind(-1)
    
    # Wasserstein distance (Gaussian approximation)
    w2 = (p_x - t_x).pow(2) + (p_y - t_y).pow(2) + \
         ((p_w - t_w).pow(2) + (p_h - t_h).pow(2)) / 4.0
         
    # Normalize to 0-1 range
    nwd = torch.exp(-torch.sqrt(w2 + 1e-7) / constant)
    return 1.0 - nwd

# --- PhD Innovation 2: Wise-IoU v3 (For All Objects) ---
def wiou_v3_loss(pred_bboxes, target_bboxes, iou_value):
    """
    Wise-IoU v3: Dynamic non-monotonic focusing mechanism.
    pred_bboxes, target_bboxes: [x, y, w, h]
    iou_value: The standard IoU (or CIoU) calculated beforehand
    """
    # 1. Distance Attention (R_WIoU)
    p_x, p_y, p_w, p_h = pred_bboxes.unbind(-1)
    t_x, t_y, t_w, t_h = target_bboxes.unbind(-1)
    
    # Small enclosing box
    w_enc = torch.max(p_x + p_w / 2, t_x + t_w / 2) - torch.min(p_x - p_w / 2, t_x - t_w / 2)
    h_enc = torch.max(p_y + p_h / 2, t_y + t_h / 2) - torch.min(p_y - p_h / 2, t_y - t_h / 2)
    
    # Distance squared
    d2 = (p_x - t_x).pow(2) + (p_y - t_y).pow(2)
    c2 = w_enc.pow(2) + h_enc.pow(2) + 1e-7
    
    R_WIoU = torch.exp(d2 / c2)
    
    # 2. Dynamic Focusing (Beta)
    # Allows model to focus on "average quality" anchors (hard but solvable)
    # and ignore low-quality ones (bad labels/impossible).
    beta = R_WIoU.detach() * iou_value.detach()
    # Gradient scaler (non-monotonic)
    r = beta / beta.mean().clamp(min=1e-7)
    
    # Final Loss
    return (1.0 - iou_value) * R_WIoU * r
...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        # --- PhD Innovation: Hybrid Loss (WIoUv3 + NWD) ---
        ...
        # 2. Convert to xywh for Custom Functions
        pbox_wh = xyxy2xywh(pbox)
        tbox_wh = xyxy2xywh(tbox)
        
        # 3. WIoU v3
        l_wiou = wiou_v3_loss(pbox_wh, tbox_wh, iou)
        
        # 4. NWD
        l_nwd = nwd_loss(pbox_wh, tbox_wh)
        
        # 5. Combine (WIoU as main, NWD as helper)
        loss_box = l_wiou + (0.5 * l_nwd)
        
        loss_iou = (loss_box.unsqueeze(-1) * weight).sum() / target_scores_sum
...
# 1. The Edge Loss Class (Sobel Filter Implementation)
class EdgeLoss(nn.Module):
    def __init__(self):
        super().__init__()
        # Sobel kernels for Edge Awareness [cite: 63]
    ...
    def forward(self, pred, target):
        ...
        if self.k_x.device != pred.device or self.k_x.dtype != pred.dtype:
             self.k_x = self.k_x.to(device=pred.device, dtype=pred.dtype)
             self.k_y = self.k_y.to(device=pred.device, dtype=pred.dtype)
        
        # Apply Sobel filters
        px, py = F.conv2d(pred_g, self.k_x, padding=1), F.conv2d(pred_g, self.k_y, padding=1)
        tx, ty = F.conv2d(target_g, self.k_x, padding=1), F.conv2d(target_g, self.k_y, padding=1)
        
        return F.l1_loss(px, tx, reduction='mean') + F.l1_loss(py, ty, reduction='mean')


def split_sr_output(preds):
    """Split the YOLO-SR training output into (detection_output, sr_image).

    During training the YOLO-SR forward pass returns a 2-tuple
    ``(detect_out, sr_out)`` where ``sr_out`` is the [B, 3, H, W] reconstruction
    produced by the Super-Resolution Auxiliary Branch (SAB). At validation /
    inference the forward pass returns the detection output only. This helper
    normalises both cases.

    Returns:
        (tuple): ``(detection_output, sr_tensor_or_None)``.
    """
    if isinstance(preds, tuple) and len(preds) == 2 and torch.is_tensor(preds[1]) and preds[1].dim() == 4:
        return preds[0], preds[1]
    return preds, None


class StructureAwareSRLoss(nn.Module):
    """Structure-Aware Multi-Task super-resolution loss for YOLO-SR (paper Sec. 3.4).

    Given the SAB reconstruction ``Î_sr`` and the input image ``I`` (both in
    [0, 1]), this returns the composite auxiliary objective (Eq. 17):

        L_sr = L_pixel^DASW + gamma * L_edge^DASW + mu * L_freq

    where:
      * ``L_pixel^DASW`` - L1 pixel fidelity (Eq. 8) weighted by the DASW mask (Eq. 16);
      * ``L_edge^DASW``  - L1 discrepancy of Sobel gradient magnitudes |Gx|+|Gy|
                           (Eq. 9-11) weighted by the same DASW mask (Eq. 16);
      * ``L_freq``       - high-pass 2D-DFT magnitude L1 (Eq. 12-14), NOT spatially
                           weighted, supplying global spectral supervision.

    Detection-Aware Spatial Weighting (DASW, Eq. 15) builds a per-pixel mask
    ``M(x, y) = 1 + eta * sum_k 1[(x,y) in b_k] * A_max / (A_k + eps)`` directly
    from the ground-truth boxes, so that smaller targets receive proportionally
    larger weight and the reconstruction gradient concentrates on the fragile
    sub-resolution objects.

    Only the SAB and this loss run during training; both are removed before
    deployment, so this term imposes no inference cost.
    """

    def __init__(self, gamma: float = 0.5, mu: float = 0.05, eta: float = 4.0,
                 a_max: float = 1024.0, eps: float = 1.0):
        """Initialize with the paper's default coefficients (Eq. 15 & 17)."""
        super().__init__()
        self.gamma = gamma          # edge-term weight  (gamma = 0.5)
        self.mu = mu                # frequency-term weight (mu = 0.05)
        self.eta = eta              # DASW foreground-emphasis (eta = 4.0)
        self.a_max = a_max          # DASW area normaliser A_max = 32x32 = 1024 px
        self.eps = eps              # DASW epsilon (prevents div-by-zero)
        # Sobel kernels (Eq. 9). Registered as buffers; device/dtype is matched
        # to the input inside each call so this works whether or not the module
        # is moved with the model.
    ...
    def _grad_mag(self, img: torch.Tensor) -> torch.Tensor:
        """Per-channel Sobel gradient magnitude |Gx| + |Gy| (Eq. 10)."""
        c = img.shape[1]
        wx = self.sobel_x.to(device=img.device, dtype=img.dtype).repeat(c, 1, 1, 1)
        wy = self.sobel_y.to(device=img.device, dtype=img.dtype).repeat(c, 1, 1, 1)
        gx = F.conv2d(img, wx, padding=1, groups=c)  # depthwise: one kernel per channel
        gy = F.conv2d(img, wy, padding=1, groups=c)
        return gx.abs() + gy.abs()

    @torch.no_grad()
    def _dasw_mask(self, batch: dict, bs: int, h: int, w: int, device, dtype) -> torch.Tensor:
        """Construct the DASW weight tensor M (Eq. 15) from ground-truth boxes.

        ``batch['bboxes']`` are normalised xywh in [0, 1]; ``batch['batch_idx']``
        maps each box to its image in the batch.
        """
        M = torch.ones((bs, 1, h, w), device=device, dtype=dtype)
        boxes = batch.get("bboxes", None)
        if boxes is None or boxes.numel() == 0:
            return M
        idx = batch["batch_idx"].long().view(-1)
        ...
        for n in range(boxes.shape[0]):
            ...
            if b < 0 or b >= bs:
                continue
            cx, cy, bw, bh = boxes[n]
            # normalised xywh -> integer pixel corners, clamped to the image
            x1 = int(torch.clamp((cx - bw / 2) * w, 0, w).item())
            x2 = int(torch.clamp((cx + bw / 2) * w, 0, w).item())
            y1 = int(torch.clamp((cy - bh / 2) * h, 0, h).item())
            y2 = int(torch.clamp((cy + bh / 2) * h, 0, h).item())
            if x2 <= x1 or y2 <= y1:
                continue  # skip degenerate boxes
            a_k = (bw * w) * (bh * h)  # box area in pixels
            weight = self.eta * self.a_max / (a_k + self.eps)  # smaller box -> larger weight
            M[b, 0, y1:y2, x1:x2] += weight
        return M

    def _freq_loss(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """High-frequency magnitude-spectrum consistency via 2D-DFT (Eq. 12-14)."""
        # FFT is computed in float32 for numerical stability (and AMP safety).
        p = pred.float()
        t = target.float()
        Fp = torch.fft.fftshift(torch.fft.fft2(p, norm="ortho"), dim=(-2, -1))
        Ft = torch.fft.fftshift(torch.fft.fft2(t, norm="ortho"), dim=(-2, -1))
        mag_diff = (Fp.abs() - Ft.abs()).abs()  # | |F{I}| - |F{Î}| |
        b, c, h, w = p.shape
        mask = self._highpass_mask(h, w, p.device)  # radial high-pass H(u, v)
        denom = (b * c * mask.sum()).clamp_min(1.0)  # C * |Omega_H| summed over the batch
        return (mag_diff * mask).sum() / denom

    @staticmethod
    def _highpass_mask(h: int, w: int, device) -> torch.Tensor:
        """Radial high-pass mask H(u, v) with r_lo = min(H, W) / 8 (Eq. 13)."""
        r_lo = min(h, w) / 8.0
        u0, v0 = h / 2.0, w / 2.0  # centre after fftshift
        yy = (torch.arange(h, device=device, dtype=torch.float32) - u0).view(h, 1)
        xx = (torch.arange(w, device=device, dtype=torch.float32) - v0).view(1, w)
        radius = torch.sqrt(yy * yy + xx * xx)
        return (radius >= r_lo).float().view(1, 1, h, w)

    def forward(self, sr_pred: torch.Tensor, batch: dict) -> torch.Tensor:
        """Compute the composite L_sr (Eq. 17) for one batch."""
        target = batch["img"]
        if target.dtype != sr_pred.dtype or target.device != sr_pred.device:
            target = target.to(device=sr_pred.device, dtype=sr_pred.dtype)
        # Align the reconstruction target to the SAB output resolution if needed.
        if target.shape[-2:] != sr_pred.shape[-2:]:
            target = F.interpolate(target, size=sr_pred.shape[-2:], mode="bilinear", align_corners=False)

        b, c, h, w = sr_pred.shape
        M = self._dasw_mask(batch, b, h, w, sr_pred.device, sr_pred.dtype)  # [B, 1, H, W]
        denom = (c * M.sum()).clamp_min(1.0)  # C * sum_xy M(x, y) over the batch (Eq. 16)

        # (a) DASW-weighted pixel fidelity (Eq. 16, L1 as in Eq. 8)
        l_pixel = (M * (target - sr_pred).abs()).sum() / denom
        # (b) DASW-weighted edge preservation on Sobel gradient magnitudes (Eq. 16)
        l_edge = (M * (self._grad_mag(target) - self._grad_mag(sr_pred)).abs()).sum() / denom
        # (c) Global frequency-domain consistency (Eq. 14) - not spatially weighted
        l_freq = self._freq_loss(sr_pred, target)

        return l_pixel + self.gamma * l_edge + self.mu * l_freq  # L_sr (Eq. 17)
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        # Fix for SR model where Detect is not last
        if not hasattr(m, 'nc') and hasattr(model.model, '__iter__'):
             for mod in model.model:
                 if hasattr(mod, 'nc') and hasattr(mod, 'reg_max') and hasattr(mod, 'cv2'):
                      m = mod
                      break
        ...
        # YOLO-SR: Structure-Aware Multi-Task super-resolution loss (paper Sec. 3.4).
        # Combines DASW-weighted pixel (Eq. 16) + edge (Eq. 16) + frequency (Eq. 14) terms.
        self.sr_loss = StructureAwareSRLoss(gamma=0.5, mu=0.05, eta=4.0)  # gamma, mu per Eq. 17
        self.lambda_sr = 0.1  # lambda_sr balances detection vs. reconstruction (Eq. 6/18)
    ...
    def __call__(
        ...
        # YOLO-SR: separate the detection output from the (training-only) SAB
        # reconstruction. At val/inference sr_pred is None (see split_sr_output).
        detect_preds, sr_pred = split_sr_output(preds)

        loss_res = self.loss(detect_preds, batch)
        loss_sum = loss_res[0].sum()

        # ============================================================
        # YOLO-SR Structure-Aware Multi-Task Loss (Eq. 6 / 17 / 18)
        # L_total = L_det + lambda_sr * (L_pixel^DASW + gamma L_edge^DASW + mu L_freq)
        # ============================================================
        if sr_pred is not None:
            l_sr = self.sr_loss(sr_pred, batch)  # composite reconstruction loss (Eq. 17)
            # loss_sum is already scaled by batch size, so scale L_sr the same way.
            batch_size = batch["img"].shape[0]
            loss_sum = loss_sum + self.lambda_sr * l_sr * batch_size

        return loss_sum, loss_res[1]
...
class E2ELoss:
    ...
    def __call__(self, preds: Any, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        """Calculate the sum of the loss for box, cls and dfl multiplied by batch size."""
        # YOLO-SR: peel off the training-only SAB reconstruction before the
        # end-to-end detection loss consumes the one2many/one2one predictions.
        det_preds, sr_pred = split_sr_output(preds)
        det_preds = self.one2many.parse_output(det_preds)
        one2many, one2one = det_preds["one2many"], det_preds["one2one"]
        loss_one2many = self.one2many.loss(one2many, batch)
        loss_one2one = self.one2one.loss(one2one, batch)
        total = loss_one2many[0] * self.o2m + loss_one2one[0] * self.o2o
        items = loss_one2one[1]

        # YOLO-SR: add lambda_sr * L_sr (Structure-Aware Multi-Task Loss, Eq. 6/18).
        # Runs only in training, when the SAB reconstruction is present.
        if sr_pred is not None:
            l_sr = self.one2one.sr_loss(sr_pred, batch)
            batch_size = batch["img"].shape[0]
            total = total + self.one2one.lambda_sr * l_sr * batch_size
