...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        # New variable to hold detection output
        detect_out = None
        ...
        for m in self.model:
            ...
            if isinstance(m, Detect): 
                 detect_out = x
                 # If inference, return detection output immediately (bypass SR)
        ...
        # If training and we have both Detect and SR output (x is SR output at the end)
        if self.training and detect_out is not None:
             return (detect_out, x)

        return x

    def strip_sr_branch(self):
        """Remove the training-only Super-Resolution Auxiliary Branch (SAB) for deployment.

        YOLO-SR keeps the SAB (the layers placed *after* the Detect head in the
        YAML) purely for training-time supervision. At inference ``_predict_once``
        already returns at the Detect head, so the SAB never executes and adds
        zero runtime FLOPs/latency. This helper additionally drops those trailing
        layers from ``self.model`` so a saved deployment checkpoint carries zero
        SAB parameters, and fuses every RepCF block into its single 3x3 conv.

        It is safe because the SAB layers are the final modules in the sequence
        and no earlier layer references them (all ``.f`` indices point backwards
        to the backbone/neck). Do NOT call this before/during training - the SAB
        must be present to provide the auxiliary reconstruction gradient.

        Returns:
            (BaseModel): self, with the SAB removed and RepCF fused.
        """
        keep = []
        for m in self.model:
            keep.append(m)
            if isinstance(m, Detect):  # stop right after the detection head
                break
        self.model = torch.nn.Sequential(*keep)
        # Any saved indices referenced only the kept (backbone/neck/head) layers.
        self.save = sorted(i for i in self.save if i < len(self.model))
        for m in self.model.modules():
            if isinstance(m, RepCF):
                m.fuse_convs()
        ...
        if not self.is_fused():
            for m in self.model.modules():
                ...
                if isinstance(m, RepConv):
                    m.fuse_convs()
                    m.forward = m.forward_fuse  # update forward
                if isinstance(m, RepCF):  # YOLO-SR: collapse RepCF to a single 3x3 conv for deployment
                    m.fuse_convs()
                    m.forward = m.forward_fuse  # update forward
...
def parse_model(d, ch, verbose=True):
    ...
    base_modules = frozenset(
        {
            ...
            A2C2f,
            HFIM,
            RepCF,  # YOLO-SR: Reparameterised Contextual Fusion (training-only, folds to one 3x3 conv)
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            c1, c2 = ch[f], args[0]
            if m is RepCF:  # YOLO-SR: RepCF is channel-preserving (identity branch needs c1==c2)
                c2 = c1
            elif c2 != nc:  # if c2 != nc (e.g., Classify() output)
                if c2 == 3: # Special case for RGB output
                     pass
                else: 
